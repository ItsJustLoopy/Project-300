...
public class UndoManager
{
    ...
    private class MoveRecord
    {
        ...
        public BlockData inventoryAsset;
        public BlockData inventoryRuntime;
    }
    ...
    public void RecordSnapshot()
    {
        ...
        if (_levelManager._playerInstance != null && _levelManager._playerScript != null)
        {
            ...
            InventoryManager inventory = _levelManager._playerScript.inventory != null
                ? _levelManager._playerScript.inventory
                : _levelManager._playerInstance.GetComponent<InventoryManager>();

            if (inventory != null && inventory.HasItem)
            {
                record.hasInventoryItem = true;
                record.inventoryAsset = inventory.HeldAsset;

                var runtimeSource = inventory.HeldRuntime != null ? inventory.HeldRuntime : inventory.HeldAsset;
                record.inventoryRuntime = runtimeSource != null ? Object.Instantiate(runtimeSource) : null;
            }
            else
            {
                record.hasInventoryItem = false;
                record.inventoryAsset = null;
                record.inventoryRuntime = null;
            }
        }
    ...
    public void UndoLastMove()
    {
        ...
        HashSet<int> levelsToRestore = new HashSet<int> { record.playerLevelIndex };
        if (record.playerLevelIndex > 0)
        {
            levelsToRestore.Add(record.playerLevelIndex - 1);
        }

        foreach (var snap in record.blockSnapshots)
        {
            if (!levelsToRestore.Contains(snap.levelIndex))
            {
                continue;
            }

            if (!_levelManager.loader.TryGetLevelObjects(snap.levelIndex, out _))
            {
                continue;
            }
        ...
        if (_levelManager._playerScript != null)
        {
            ...
            InventoryManager inventory = _levelManager._playerScript.inventory != null
                ? _levelManager._playerScript.inventory
                : (_levelManager._playerInstance != null ? _levelManager._playerInstance.GetComponent<InventoryManager>() : null);

            if (inventory != null)
            {
                if (record.hasInventoryItem)
                {
                    BlockData runtimeSnapshot = record.inventoryRuntime != null
                        ? Object.Instantiate(record.inventoryRuntime)
                        : (record.inventoryAsset != null ? Object.Instantiate(record.inventoryAsset) : null);

                    inventory.SetHeldFromSave(record.inventoryAsset, runtimeSnapshot);
                }
                else
                {
                    inventory.Clear();
                ...
                _levelManager._playerScript.inventory = inventory;
            }
        }
        ...
        foreach (int levelIndex in _levelManager.loader.GetLoadedLevelIndices())
        {
            float opacity = levelIndex == _levelManager.currentLevelIndex
                ? 1f
                : _levelManager.inactiveLevelOpacity;
    ...
    public void ClearHistoryForLevel(int levelIndex)
    {
        for (int i = _undoStack.Count - 1; i >= 0; i--)
        {
            if (_undoStack[i].playerLevelIndex == levelIndex)
            {
